...
extern "C" int main(int argc, const char *argv[]) {
  ...
  if (!FLAGS_split_png_atlas.empty()) {
    Singleton<AtlasFontEngine>::Get()->Init(FontDesc(FLAGS_split_png_atlas, "", 0, Color::white, Color::clear, 0));
    ...
    for (auto b = font->glyph->table.begin(), e = font->glyph->table.end(), i = b; i != e; ++i) {
      if (i->tex.width > 0 && i->tex.height > 0) { CHECK(i->advance > 0) glyph_index[v4(i->tex.coord)] = i->id; }
    }
    ...
    string atlas_png_fn = StrCat(app->assetdir, FLAGS_split_png_atlas, ",0,255,255,255,0.0000.png");
